...
//사용자 입력을 받는 요소를 찾아 변수에 저장
//이벤트 처리
document.addEventListener("DOMContentLoaded", getLocal)
addButton.addEventListener("click", addTodo)
todoList.addEventListener("click", manageTodo)//todo리스트에서 이벤트 발생시
//추가 버튼을 찾아 변수에 저장
//함수
function addTodo (e){   //addTodo 함수 정의 시작과 기본 동작을 취소
...
const newTodo = document.createElement('li');    //생성한 li 요소에 todo-content 클래스 추가
newTodo.innerText = todoInput.value;
newTodo.classList.add('todo-content');
newDIV.appendChild(newTodo); //새로운 내용추가

saveToLocal(todoInput.value);//새로운 로컬 생성

const completeButton = document.createElement('button');    //'button' 요소 생성하고 완료 텍스트를 성정한 뒤 complete-button 클래스 추가
...
deleteButton.innerText = '이승기 - 삭제';   //삭제 text
deleteButton.classList.add('delete-button'); //delete-button 클래스 추가
newDIV.appendChild(deleteButton);           //div요소에 생성한 삭제 버튼을 자식 요소로 추가

todoList.appendChild(newDIV);//ul요소의 자식 요소 추가

todoInput.value = "";   //입력 창 초기화
}

function saveToLocal(todo){ //saveToLocal 함수를 정의하는 부분의 시작 사용자가 입력한 값을 로컬 스톨리지에 저장하는 역할
    let todos;
    if(localStorage.getItem('todos') === null){ //저장된 항목이 없으면 빈배열 생성
        todos = [];     
    } else {
...
//로컬스토리지에 저장된 todo 항목을 가져와 화면에 표시하는 역할을 함
...
function getLocal(){
    ...
    todos.forEach(function(todo){   //todos 요소마다 반복
        ...
        newTodo.innerText = todo;   //로컬 스토리지으 ㅣ값 표시
        newTodo.classList.add('todo-content');
        newDiv.appendChild(newTodo);

        const completeButton = document.createElement('button');    //완료 버튼 만듬
        completeButton.innerText = '완료';
        completeButton.classList.add('complete-button');
        newDiv.appendChild(completeButton);

        const deleteButton = document.createElement('button');      //삭제버튼
        deleteButton.innerText = '이승기 - 삭제';
...
function manageTodo(e){     //to-do 리스트에서 발생한 이벤트를 처리하는 역할
    ...
    }else if(whichButton === 'delete-button'){  //클릭한 부분이 삭제 이면
        const todo = e.target.parentElement;
        removeLocal(todo); //항목제거
        todo.remove();
    }
}   //to-do 항목을 로컬 스토리지에서 제거하는 역할을 함
function removeLocal(todo){
    let todos;  //로컬스토리지에서 가져온 할 일들
    if(localStorage.getItem('todos')===null){
        todos = [];
    }else{
        todos = JSON.parse(localStorage.getItem('todos'));      //삭제 to-do 항목의 인덱스 찾음
    }
